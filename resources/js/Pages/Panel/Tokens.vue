...
const crear = () => form.post('/panel/tokens', { onSuccess: () => form.reset() });

const revocar = (id) => {
    if (confirm('¿Revocar este token? Las integraciones que lo usen dejarán de funcionar.')) {
        router.delete(`/panel/tokens/${id}`);
    }
};
...
<template>
    ...
    <PanelLayout>
        ...
        <div
            ...
            class="mb-6 rounded-xl border border-indigo-200 bg-indigo-50 p-4"
        >
        ...
        <form class="mb-6 flex items-end gap-3" @submit.prevent="crear">
            <div class="flex-1">
                <label for="nombre" class="mb-1 block text-sm font-medium text-gray-700">
                    Nombre del token (p. ej. «ERP contabilidad»)
                </label>
                <input
                    id="nombre"
                    v-model="form.nombre"
